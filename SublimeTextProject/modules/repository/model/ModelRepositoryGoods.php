<?php 
	class ModelRepositoryGoods extends BaseModel{
		...
		function decreaseAmount($amount){ 
			...
			if($amount){
				$result =  $this->decreasePropertyValueToDb($amount_index,$amount);
				if(!$result)
					throw new Exception(self::$TAG."Decrease Amount failed! ", 1);
			}
			else{
				throw new Exception(self::$TAG."Decrease Amount failed, Amount == 0 !", 1);
			}
		}
	}
?>